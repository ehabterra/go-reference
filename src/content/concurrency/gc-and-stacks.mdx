...
<Playground client:visible title="pool.go" code={String.raw`package main
...
The pooled loop does the same work with **near-zero** allocations — that's the GC pressure you've removed. See the [`sync` package](/concurrency/sync-package/) for the full API and when pooling actually pays off (it can *hurt* for cheap or rarely-reused objects).
