...
<Challenge client:visible id="runes-count" title="rune-count.go"
  brief="len(word) counts bytes, and é takes two bytes in UTF-8. Count the letters (runes), not the bytes."
  expected="letters in héllo: 5"
  hint="The unicode/utf8 package has a function that counts runes in a string. Converting to []rune and taking len of that works too."
...
func main() {
	word := "héllo"
	fmt.Println("letters in", word+":", utf8.RuneCountInString(word))
}`} />
