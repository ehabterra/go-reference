...
<Challenge client:visible id="maps-nil-write" title="nil-map.go"
  brief="The map is declared but never initialized, so the write panics. Make the program run and print the expected output."
  expected="13"
  hint="A nil map behaves like an empty map for reads, but any write panics. Something needs to allocate the map before the assignment — make() or a map literal."
