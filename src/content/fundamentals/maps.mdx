...
The standard `maps` package adds the operations the language leaves out. Note that in Go 1.23+ `maps.Keys` and `maps.Values` return an **[iterator](/fundamentals/iterators/)** (`iter.Seq`), not a slice — collect with `slices.Collect` and `slices.Sort` it when you need order:
