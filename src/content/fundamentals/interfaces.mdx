...
<Challenge client:visible id="interfaces-typed-nil" title="typed-nil.go"
  brief="validate returns a nil *validationError, but the caller's err != nil check still fires — the interface holds a type, so it isn't nil. Make valid input report as valid."
  expected="valid input"
  hint="Returning a nil typed pointer wraps it in a non-nil interface. When there is no error, return the literal nil — don't funnel the result through a typed variable."
...
func validate(input string) error {
	if input == "" {
		return &validationError{"input is empty"}
	}
	return nil // a literal nil interface, not a typed nil pointer
}
