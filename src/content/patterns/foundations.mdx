...
quiz:
  ...
  - q: "Which single principle underlies nearly every design pattern?"
    options:
      - text: "Encapsulate what varies — find the changing part and separate it from what stays stable"
        correct: true
      - text: "Make everything global"
        correct: false
      - text: "Always use inheritance"
        correct: false
      - text: "Avoid interfaces"
        correct: false
    explain: "Almost every pattern isolates the volatile part of a design behind a stable boundary: Strategy isolates an algorithm, Decorator isolates added behavior, Factory isolates construction. 'Encapsulate what varies' is the seed most patterns grow from."
  - q: "A teammate wraps everything in patterns 'to be flexible.' What does the book warn?"
    options:
      - text: "Patterns add indirection and cost; apply them when a real, recurring problem calls for one — over-using them is itself an anti-pattern"
        correct: true
      - text: "You can never use too many patterns"
        correct: false
      - text: "Patterns make code faster, so always use them"
        correct: false
      - text: "Patterns are required in Go"
        correct: false
    explain: "Patterns earn their keep against actual change and complexity. Forcing them speculatively ('pattern fever') adds layers, names, and indirection with no payoff — the simplest thing that works, especially in Go, often beats a formal pattern."
---
