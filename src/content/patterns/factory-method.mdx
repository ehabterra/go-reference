...
quiz:
  ...
  - q: "What does the 'registry' factory variation buy you over a hard-coded switch?"
    options:
      - text: "New implementations register themselves (often in init) without editing the factory — it's open for extension"
        correct: true
      - text: "It runs measurably faster than a switch"
        correct: false
      - text: "It removes the need for interfaces"
        correct: false
      - text: "It only works with generics"
        correct: false
    explain: "A registry maps a name to a constructor func. New kinds call Register from their own package's init(), so the factory never changes when you add one — exactly how image.RegisterFormat and sql.Register work."
  - q: "Factory Method vs Abstract Factory — what's the key difference?"
    options:
      - text: "Factory Method creates one product through an overridable step; Abstract Factory creates whole families of related products"
        correct: true
      - text: "They are the same pattern"
        correct: false
      - text: "Abstract Factory can't return interfaces"
        correct: false
      - text: "Factory Method requires inheritance in Go"
        correct: false
    explain: "Factory Method varies the creation of a single product per context. Abstract Factory groups several related creation methods so you get matched sets (NY dough + NY sauce, or Chicago dough + Chicago sauce) from one factory."
---
...
<Playground client:visible title="factory.go" code={String.raw`package main
