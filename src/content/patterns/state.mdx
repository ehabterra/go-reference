...
quiz:
  ...
  - q: "What is the idiomatic 'function-based' state machine in Go (Rob Pike's lexer style)?"
    options:
      - text: "A state is a function that does work and returns the next state: type stateFn func(*M) stateFn"
        correct: true
      - text: "A goroutine per state"
        correct: false
      - text: "A global map of booleans"
        correct: false
      - text: "A single giant switch with no functions"
        correct: false
    explain: "Instead of a type per state, each state is a function returning the next state function; the driver loops `state = state(m)` until it returns nil. text/template and go/scanner use this exact shape — it's the most Go-native State machine."
  - q: "When does a struct-per-state (or stateFn) beat a plain switch on a state field?"
    options:
      - text: "When states multiply, each carries its own behavior, and the transition logic would otherwise be scattered across many methods' switches"
        correct: true
      - text: "Always — switches are never acceptable"
        correct: false
      - text: "Only for exactly two states"
        correct: false
      - text: "Never — a switch is always better"
        correct: false
    explain: "For two states and one transition, a bool + if wins. The pattern pays off when behavior-per-state grows and the same switch(state) would otherwise be duplicated across every method — then localizing each state's behavior and transition into its own type/function removes the scatter."
---
...
<Playground client:visible title="state.go" code={String.raw`package main
...
<Playground client:visible title="statefn.go" code={String.raw`package main
