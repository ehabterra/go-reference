...
quiz:
  ...
  - q: "Which design principle does a Facade most directly serve?"
    options:
      - text: "Least Knowledge (Law of Demeter) — clients talk to one friend instead of reaching deep into many subsystems"
        correct: true
      ...
      - text: "Open/Closed, exclusively"
        correct: false
    explain: "The Law of Demeter says 'talk only to your immediate friends.' A facade gives clients exactly one friend to talk to, so they don't chain calls into the guts of inventory, payment, and shipping — cutting coupling to the subsystem's internals."
  - q: "Facade vs Mediator — how do they differ?"
    options:
      - text: "Facade is a one-way simplified front over a subsystem; Mediator coordinates two-way interaction between peer objects that would otherwise talk directly"
        correct: true
      - text: "They are identical"
        correct: false
      - text: "Mediator is creational"
        correct: false
      - text: "Facade requires goroutines"
        correct: false
    explain: "A facade points one way: client → facade → subsystem, and the subsystem doesn't know the facade exists. A mediator sits between colleagues that send messages back through it, decoupling a many-to-many mesh into a hub."
---
...
<Playground client:visible title="facade.go" code={String.raw`package main
...
The Facade is the clearest expression of the **Principle of Least Knowledge** — *talk only to your immediate friends*. Without it, a caller chains into the guts of every subsystem (`order.Inventory().Warehouse(3).Reserve(...)`), coupling itself to structures it shouldn't know. The facade gives the client **one friend** to call, so the subsystem's shape can change freely behind it.

That's also the line between Facade and its neighbours: a [Mediator](/patterns/mediator/) coordinates peers that talk *back* through it (two-way), while a facade is a one-way simplification whose subsystems don't even know it exists; an [Adapter](/patterns/adapter/) changes *one* type's interface rather than simplifying *many*.
