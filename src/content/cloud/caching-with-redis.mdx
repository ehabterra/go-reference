...
**Redis** is an in-memory key–value store — data lives in RAM, so reads and writes are sub-millisecond — commonly used as a cache in front of a slower database (and also for sessions, rate limiters, and queues). The most common caching pattern is **cache-aside (lazy loading)**: the app checks the cache, and on a miss loads from the database and populates the cache.
