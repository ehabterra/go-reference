...
quiz:
  ...
  - q: "Why prefer the ...Context query methods (QueryContext, ExecContext) over the plain ones?"
    options:
      - text: "If the request's context is cancelled or times out, the query is cancelled too — freeing the connection instead of blocking on a dead client"
        correct: true
      - text: "They're faster"
        correct: false
      - text: "They auto-retry failed queries"
        correct: false
      - text: "They escape the SQL for you"
        correct: false
    explain: "Passing the request's context lets a cancelled/timed-out request abort the in-flight query and return its pooled connection, instead of holding it until the DB finishes. Always thread ctx from the handler down to QueryContext/ExecContext/QueryRowContext."
  - q: "How do you Scan a column that might be NULL into a Go value?"
    options:
      - text: "Use a nullable wrapper — sql.NullString / sql.NullInt64 / sql.Null[T] — and check its .Valid field"
        correct: true
      - text: "Scan into a plain string; NULL becomes \"\""
        correct: false
      - text: "NULL columns can't be scanned at all"
        correct: false
      - text: "Use a *string and ignore nil"
        correct: false
    explain: "Scanning SQL NULL into a plain string/int errors. The sql.Null* types (and the generic sql.Null[T] in Go 1.22+) hold both the value and a Valid bool: if Valid is false the column was NULL. A *string pointer also works (nil = NULL), but the Null types are clearer."
---
...
- [SQL transactions](/web/sql-transactions/) — grouping statements into all-or-nothing units.
- [Postgres & Redis](/web/postgres-and-redis/) — picking a driver and a datastore.
- [web security](/web/web-security/) — parameterized queries as injection defense.
- [project layout & DI](/web/project-layout-and-di/) — injecting the `*sql.DB` into your services.
