...
difficulty: "Beginner"
status: ready
intent: "Connectionless datagrams in Go — net.ListenPacket and ReadFrom/WriteTo, DialUDP, message boundaries with no delivery guarantees, encoding a datagram payload, MTU and fragmentation, and when UDP beats TCP."
related:
  - tcp-sockets
...
quiz:
  ...
  - q: "Why does a UDP server use ReadFrom rather than Read?"
    options:
      - text: "There's no connection, so ReadFrom also returns the sender's address — you need it to reply"
        correct: true
      - text: "Read doesn't exist for UDP sockets"
        correct: false
      - text: "ReadFrom is faster"
        correct: false
      - text: "ReadFrom decrypts the datagram"
        correct: false
    explain: "A single UDP socket serves every client — there's no per-connection object. ReadFrom hands back both the bytes and the *net.UDPAddr they came from, so WriteTo can send the reply to the right peer."
  - q: "Why keep UDP datagrams small (often ≤ ~1400 bytes)?"
    options:
      - text: "A datagram larger than the path MTU is IP-fragmented, and losing any fragment drops the whole datagram"
        correct: true
      - text: "Go refuses to send datagrams over 1400 bytes"
        correct: false
      - text: "Larger datagrams are encrypted differently"
        correct: false
      - text: "UDP has a hard 1400-byte limit in the spec"
        correct: false
    explain: "If a datagram exceeds the path MTU it's split into IP fragments; if any fragment is lost, the entire datagram is discarded, so loss probability rises sharply. Staying under ~1400 bytes keeps each datagram to a single packet."
---
...
<Playground client:visible title="datagram.go" code={String.raw`package main
...
Because UDP preserves message boundaries, one datagram = one of these records — no framing needed (that's TCP's problem). See [framing & protocols](/web/framing-and-protocols/) for the binary-encoding details.
...
- **Multicast** — `net.ListenMulticastUDP` sends one datagram to a whole group; TCP can't multicast at all.
...
- [TCP sockets](/web/tcp-sockets/) — the reliable, ordered, connection-based alternative.
- [framing & custom protocols](/web/framing-and-protocols/) — `encoding/binary` and laying out a binary message.
- [DNS & addressing](/web/dns-and-addressing/) — DNS is the classic UDP application.
- [fmt & io](/stdlib/fmt-and-io/) — the Reader/Writer model the `net` package shares.
